export const words = ["Full Stack Developer", "Software Engineer", "React Specialist", "Backend Developer", "Problem Solver"];

export const projects = [
    {
      title: 'E-Commerce Platform',
      description: 'Full-stack e-commerce solution with React, Node.js, and PostgreSQL featuring real-time inventory management.',
      icon: '🛒',
      tech: ['React', 'Node.js', 'PostgreSQL', 'Stripe'],
      github: '#',
      demo: '#'
    },
    {
      title: 'Task Management System',
      description: 'Collaborative project management tool with real-time updates, file sharing, and team communication.',
      icon: '📋',
      tech: ['React', 'TypeScript', 'Socket.io', 'MongoDB'],
      github: '#',
      demo: '#'
    },
    {
      title: 'Real-Time Chat Application',
      description: 'Scalable chat platform with WebSocket integration, message encryption, and multimedia support.',
      icon: '💬',
      tech: ['React Native', 'WebSocket', 'Redis', 'JWT'],
      github: '#',
      demo: '#'
    },
    {
      title: 'Analytics Dashboard',
      description: 'Business intelligence dashboard with interactive charts, data visualization, and export capabilities.',
      icon: '📊',
      tech: ['React', 'D3.js', 'Python', 'FastAPI'],
      github: '#',
      demo: '#'
    },
    {
      title: 'API Gateway Service',
      description: 'Microservices API gateway with rate limiting, authentication, and monitoring built with Golang.',
      icon: '🔗',
      tech: ['Golang', 'Docker', 'Kubernetes', 'Redis'],
      github: '#',
      demo: '#'
    },
  ];

export const skills = [
  {
    title: "Frontend Technologies",
    skills: [
      { name: "React.js", level: 90 },
      { name: "TypeScript", level: 85 },
      { name: "JavaScript", level: 95 },
      { name: "React Native", level: 80 }
    ]
  },
  {
    title: "Backend Technologies",
    skills: [
      { name: "Golang", level: 90 },
      { name: "C#/.NET Core", level: 85 },
      { name: "Node.js", level: 80 },
      { name: "Python", level: 75 },
    ]
  },
  {
    title: "Database & Tools",
    skills: [
      { name: "PostgreSQL", level: 85 },
      { name: "MongoDB", level: 80 },
      { name: "Redis", level: 75 },
      { name: "Docker", level: 80 }
    ]
  }
];

export const experiences = [
  {
    role: "Senior Full Stack Developer",
    company: "Tech Solutions Inc.",
    location: "Remote",
    duration: "Jan 2023 - Present",
    description: "Leading development of scalable web applications and microservices architecture, mentoring junior developers, and implementing best practices for code quality and performance optimization.",
    responsibilities: [
      [
        "Architected and developed 5+ full-stack applications using React and Golang",
        "Implemented microservices architecture reducing system latency by 40%",
        "Led code reviews and established development best practices"
      ],
      [
        "Mentored 3 junior developers and conducted technical interviews",
        "Optimized database queries resulting in 60% performance improvement",
        "Collaborated with product team to define technical requirements"
      ]
    ],
    tech: ["React", "TypeScript", "Golang", "PostgreSQL", "Docker", "Kubernetes", "AWS"]
  }
];

export const achievements = [
  {
    title: "First Prize Winner",
    description: "National Level Hackathon 2023",
    icon: "🏆",
    year: "2023"
  },
  {
    title: "AWS Certified",
    description: "Solutions Architect Associate",
    icon: "☁️",
    year: "2023"
  },
  {
    title: "Open Source Contributor",
    description: "500+ contributions on GitHub",
    icon: "🌟",
    year: "2022"
  },
  {
    title: "Tech Speaker",
    description: "Presented at 3 tech conferences",
    icon: "�",
    year: "2022"
  }
];

export const education = [
  {
    degree: "Bachelor of Technology",
    field: "Computer Science Engineering",
    institution: "XYZ University",
    location: "India",
    duration: "2019 - 2023",
    grade: "8.5 CGPA",
    description: "Specialized in software engineering, data structures, algorithms, and web development. Active member of coding club and technical societies."
  }
];

export const stats = [
  { label: "Years Experience", value: "2+", icon: "💼" },
  { label: "Projects Completed", value: "15+", icon: "🚀" },
  { label: "Happy Clients", value: "10+", icon: "😊" },
  { label: "Code Commits", value: "1000+", icon: "💻" }
];

export const socialLinks = [
  {
    name: "LinkedIn",
    url: "https://www.linkedin.com/in/sudhansh-goel-867349270/",
    icon: "linkedin"
  },
  {
    name: "GitHub",
    url: "https://github.com/sudhanshgoel",
    icon: "github"
  },
  {
    name: "Twitter",
    url: "https://twitter.com/sudhanshgoel",
    icon: "twitter"
  },
  {
    name: "Email",
    url: "mailto:sudhansh.goel@example.com",
    icon: "email"
  }
];

export const tags = [
  'RESTful APIs', 'Microservices', 'Third-party Integrations', 'Auth0', 'Twilio', 'Stripe', 'Supabase', 'PWA', 'CI/CD', 'Performance Optimization'
];

export const icons = [
  {
    name: "Instagram",
    href: "https://www.instagram.com/sudhanshgoel1?igsh=MW1sbDhuM3M4Y3RuZA==",
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="30"
        height="30"
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
      >
        <rect x="2" y="2" width="20" height="20" rx="5" ry="5" />
        <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
        <line x1="17.5" y1="6.5" x2="17.51" y2="6.5" />
      </svg>
    ),
  },
  {
    name: "facebook",
    href: "https://www.facebook.com/share/16Q9QHXpC7/",
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="40"
        height="40"
        fill="currentColor"
        viewBox="0 0 16 16"
      >
        <path d="M8.94 6.79H7.54V5.54c0-.4.27-.49.46-.49h.9V3.5h-1.2C6.44 3.5 6 4.54 6 5.38v1.41H5v1.7h1v4.5h1.54v-4.5h1.21l.19-1.7z" />
      </svg>
    ),
  },
  {
    name: "twitter",
    href: "https://x.com/sudhanshgoel1",
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="30"
        height="30"
        fill="currentColor"
        viewBox="0 0 16 16"
      >
        <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.007-.423A6.673 6.673 0 0 0 16 3.542a6.574 6.574 0 0 1-1.889.518A3.301 3.301 0 0 0 15.555 2a6.533 6.533 0 0 1-2.084.797A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429A3.284 3.284 0 0 0 2.13 7.16a3.323 3.323 0 0 1-1.487-.41v.041c0 1.566 1.11 2.873 2.583 3.168a3.273 3.273 0 0 1-1.482.056c.418 1.306 1.63 2.256 3.067 2.283A6.588 6.588 0 0 1 0 13.557 9.29 9.29 0 0 0 5.026 15z" />
      </svg>
    ),
  },
  {
    name: "linkedin",
    href: "https://www.linkedin.com/in/sudhansh-goel-867349270?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app",
    svg: (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="30"
        height="30"
        fill="currentColor"
        viewBox="0 0 16 16"
      >
        <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175A1.145 1.145 0 0 1 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.354-.554 1.354-1.248-.015-.71-.517-1.249-1.339-1.249-.822 0-1.353.54-1.353 1.249 0 .694.516 1.248 1.322 1.248h.016zM13.458 13.394v-4.045c0-2.168-1.162-3.177-2.713-3.177-1.249 0-1.806.686-2.122 1.17v-1.004H6.222c.029.66 0 7.225 0 7.225h2.401v-4.037c0-.216.015-.432.08-.586.176-.432.576-.88 1.248-.88.88 0 1.233.664 1.233 1.637v3.866h2.274z" />
      </svg>
    ),
  },
];
